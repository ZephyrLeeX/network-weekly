...
def test_defaults_match_spec() -> None:
    """§14/§15.3: >= 80% for two consecutive valid samples."""
...
def test_validate_rejects_out_of_range_samples() -> None:
    ...
    with pytest.raises(ThresholdValueError):
        validate_threshold(REQUIRED_SAMPLES_KEY, 10_001.0)
...
def test_cycle_slots_aligned_and_half_open() -> None:
    ...
    assert cycle_slots(start, end, five_minutes) == [
    ...
    assert cycle_slots(T0, T0 + timedelta(minutes=5), five_minutes) == [T0]

    # No slots when the window is empty.
    assert cycle_slots(T0, T0, five_minutes) == []

    assert len(cycle_slots(T0, T0 + timedelta(days=7))) == 336
    assert len(cycle_slots(T0, T0 + timedelta(days=7), timedelta(hours=1))) == 168
