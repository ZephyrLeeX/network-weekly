...
240 s whole-poll deadline covering SNMP plus its SSH reachability fallback;
propagate interval into reachability continuity,
...
**Acceptance:** focused SSH/SNMP/session 61 PASS and focused poll-device
integration 8 PASS; full non-integration 407 PASS; integration 267 PASS; ruff
PASS; mypy PASS (137 files); Alembic 0011 with no migration. SNMP and SSH share
one absolute monotonic deadline. SSH connect/read native timeouts are capped by
fresh remaining budgets; expiry starts no new request, preserves successful
sections, leaves SSH reachability unknown, and is not Down/Recovery evidence.
Non-DEVICE_POLL SSH keeps its configured timeout behavior. Field revalidation
remains mandatory before REVIEW_PASSED.
**Implementation checkpoint:** `213ac4983325d8288fc53ac1fd78ded2a3172a68`.
**Previous low-impact polling checkpoint:**
`4c4466e13c23937934f11befaac1f6592f549ba9`.
...
**Status:** IMPLEMENTED — FIELD_REVALIDATION_PENDING (2026-09-22; release-blocking, not Wave-2-blocking)
**Depends On:** W01-T006
...
## W05-AUDIT — Wave 5 audit hotfix
**Status:** REVIEW_FAILED — superseded by W05-AUDIT-2（2026-09-06, branch work/wave-05）。History preserved below: the entry WAS marked REVIEW_PASSED on 2026-09-06 and its fixes re-tested green, but its fix (1) was subsequently found defective — the heartbeat baseline state machine had NO container-identity input, so the OLD worker's post-baseline final tick could pass as evidence that a REPLACED worker was healthy (baseline → old worker ticks again → compose stops it → replacement worker never starts its heartbeat loop → `last_heartbeat > baseline`, seconds fresh → false PASS). That finding is RESOLVED and re-verified by W05-AUDIT-2 (REVIEW_PASSED). Fixes (2) retention evidence and (3) schema-aware rollback guidance are unaffected and remain standing.  
**Depends On:** W05-T005 (scaffolding)  
**Blocks:** none (audit only; no product capability added, NO new migration — alembic head stays 0011)  
**Scope:** three audit findings — (1) heartbeat verifier false positive: `deploy/lib.sh::wait_heartbeat` accepted ANY fresh `worker_heartbeat` row, so a heartbeat left behind by the PREVIOUS worker (even seconds old) passed as evidence that the newly installed/updated worker had started; (2) acceptance evidence error: `scripts/acceptance_evidence.sh` "oldest raw data" section computed `MAX(collected_at)` and labelled it "oldest", so the normal presence of recent rows always masked data retained beyond 90 days (A13 could never fail); (3) unsafe rollback guidance: `deploy/update.sh` printed an unconditional `rollback with: update.sh --image <previous>` for restart/health/heartbeat failures (17/18/19) even though the migration had ALREADY committed and the previous image may no longer run against the migrated schema.  
**Implementation:**
- Heartbeat verifier state machine (new `backend/ops/heartbeat_verify.py`, used by `deploy/lib.sh`): the verifier accepts only evidence written AFTER the start/update being validated — before `up -d`, install.sh/update.sh capture the baseline of `NETWORK_REPORT_WORKER_ID` (`{"last_heartbeat", "started_at"}` or `{}`) via a one-off container on the TARGET image; after the start the verifier demands: the row for THIS worker id exists (query scoped by `worker_id` — a fresh row of a DIFFERENT worker id is invisible and never passes; the old `select(...).limit(1)` arbitrary-row query is gone), `last_heartbeat` strictly newer than the baseline (`{}` baseline = first row appearing is the evidence, fresh install), and age `< interval*4`; `started_at` is reported as supplementary evidence ("worker restarted / kept running") but is deliberately NOT a pass condition so a same-image in-place re-verify (worker not recreated) stays legal. An equivalent inline fallback in lib.sh keeps rollback targets that predate the module usable (`update.sh --image <old>`; the smoke matrix caught an `sys.argv[2]` index bug in exactly that fallback — fixed).
- Retention evidence (new `backend/ops/retention_evidence.py`, replacing the section in `scripts/acceptance_evidence.sh`): reports the TRUE oldest row (`MIN(collected_at)` / `MIN(cycle_started_at)`) and its age in days for `device_metrics` / `interface_metrics` / `device_poll_runs`, with the A13 verdict `OK` (≤ 91d slack) / `BEYOND-90d`; docs/ACCEPTANCE.md A13 wording aligned ("最老行（MIN…）年龄 ≤ ~90 天"). The W05-T004 retention implementation itself is untouched (§24 loop unchanged).
- Rollback semantics (`deploy/lib.sh::post_migration_failure` + update.sh): all three post-migration failure paths (17 restart / 18 health / 19 heartbeat) now print the schema-aware remediation — "Database migration has already committed. The schema may no longer be compatible with the previous image. Do NOT blindly roll back the application image. See docs/OPERATIONS.md rollback procedure. If the previous image cannot run against the migrated schema, restore the pre-update database backup first, then start the old image." — and the `update complete` hint is schema-qualified. The migration-not-committed path is unchanged and safe (14: `.env` restored, nothing restarted). `docs/OPERATIONS.md` §0/§4/§5 rewritten accordingly: pre-update pg_dump; migration not committed → rollback safe; migration committed → check compatibility, restore the pre-update backup FIRST when incompatible; the system never auto-downgrades or auto-restores the database.
**Tests:** +8 unit (`test_ops_heartbeat_verify.py`: scenarios A stale-leftover FAIL / restart-alone-insufficient / B other-worker-invisible / C new-write PASS / D fresh-install PASS / E same-image must observe a new write, strict-newness, age limit, baseline JSON round-trip and malformed input, CLI usage; `test_ops_retention_evidence.py`: 91d threshold, oldest-row line BEYOND-90d vs OK, empty table; `test_deploy_scripts.py` +3: worker-id-only verifier + baseline-before-start ordering pinned, post-migration paths schema-aware with `"rollback with:"` absent from update.sh entirely, and `post_migration_failure` EXECUTED for 17/18/19 asserting the mandated block); +10 integration (`test_heartbeat_verify_persistence.py`: scenarios A–E against real PostgreSQL incl. worker-id scoping, TIMESTAMPTZ microsecond baseline round trip; `test_retention_evidence_oldest.py`: seeded 180d+1d rows in all three raw tables → oldest_rows returns MIN=180d (never MAX), rendered evidence = 3× BEYOND-90d with no OK, recent-only → OK, empty → `: empty`). Suite: **318 unit + 253 integration PASS**, ruff clean, mypy clean (133 files), `alembic upgrade head` idempotent at **0011 = head** (no new migration).  
**Acceptance (smoke evidence, staging roots on Debian 13 amd64, throwaway stack, image built from this commit's tree):** fresh install exit 0 with `heartbeat baseline: {}` → first-heartbeat PASS (D); all-container force-recreate → 10 devices + fresh post-restart heartbeat intact; **stale-leftover scenario (A): verifier FAIL with "a stale row from a previous worker must not pass"** (the pre-audit check PASSED here — the fixed false positive); other-worker scenario (B): with only a different worker id's 18s-fresh row present, verifier FAIL "no heartbeat row for worker 'worker'"; new-write scenario (C): restarted worker → PASS "worker restarted"; same-image update (E): the in-place re-verify REJECTED the baseline-identical row twice and passed only on the running worker's next tick ("kept running") — no reuse of the old value; REAL image update `w04t1-smoke(0009)` → `HEAD` ran the migration live 0009→0010→0011, exit 0, 10 devices + admin intact, new-heartbeat verified; raising-0012 image → exit 14, `.env` restored, zero recreates (stack untouched, verified via StartedAt); migration-success + restart-failure messaging triggered FOR REAL via a port conflict after a committed migration → exit 17 printed the mandated schema-aware block with ZERO `rollback with:` lines (18/19 pinned by executing the same helper); documented rollback loop (explicit operator downgrade of the throwaway staging DB + `update.sh --image prev`) → exit 0 through the legacy fallback; retention evidence scenario: seeded 180d + 1d rows → all three tables `oldest row … age 180.0 days BEYOND-90d` (recent row no longer masks the old data); full collector run exit 0.  
**Control state:** W05-T001..T004 remain REVIEW_PASSED (revalidated); W05-T005 remains **IN_PROGRESS — REAL_ENVIRONMENT_EVIDENCE_PENDING** (A01–A16 all PENDING, no real-environment evidence fabricated); W01-T007 BLOCKED — FIELD_VALIDATION_PENDING; W03-T010 BLOCKED — REAL_WEEK_DATA_PENDING; W05-GATE **BLOCKED**; no formal two-week acceptance timing started; `main` NOT merged.  
**Checkpoint:** see EXECUTION_STATE.md Wave 5 checkpoint ledger (838b5b613f80ed60999018ca8e061b798e81a51e).

## W05-AUDIT-2 — Wave 5 audit follow-up
**Status:** REVIEW_PASSED（2026-09-06, branch work/wave-05; W05-T001..T004 revalidated PASS against the fixes）  
**Depends On:** W05-AUDIT（supersedes its fix 1）  
**Blocks:** none (audit only; no product capability added, NO new migration — alembic head stays 0011)  
**Scope:** two substantive findings + two doc typos — (1) heartbeat verifier race (supersedes W05-AUDIT fix 1): a baseline captured before `compose up` cannot by itself attribute a later heartbeat to the NEW worker, because the old worker keeps running for a while and its final tick (`last_heartbeat` > baseline, seconds fresh) lands after the baseline but before compose stops it — the W05-AUDIT state machine accepted that as "new worker healthy" while the replacement worker had never written anything; (2) A09 evidence error: docs/ACCEPTANCE.md proved "连续 2 个周一 00:10 自动报告" from `weekly_reports.generated_at`, but that registry holds the CURRENT success per week — a later manual regenerate overwrites `generated_at`, and `WeeklyReport` itself cannot prove `trigger == scheduled`; plus the two typos 人工核对类条目（A12/A13）→（A15/A16） and A15's 证据来源 "A13 模板人工核对记录" → "A15 模板人工核对记录".  
**Implementation:**
- Container-identity state machine (`backend/ops/heartbeat_verify.py`): `verify_heartbeat` now receives whether the worker CONTAINER was replaced (`is_container_replaced(pre_id, post_id)` over full `docker inspect` IDs); the CLI is `verify <baseline> <pre-container-id> <post-container-id>`. Rules: the row for THIS worker id must exist (worker row missing → FAIL); with a baseline `last_heartbeat` must be strictly newer (baseline-identical → FAIL) and fresh (`age < interval*4`, stale → FAIL); a baseline of `{}` (fresh install) still passes on the first fresh row — `started_at` has no baseline to move past. ONLY when the container was replaced must `started_at` ALSO be newer than the baseline's: `last_heartbeat > baseline` with `started_at == baseline.started_at` under replacement FAILs with the explicit reason "old worker wrote after the baseline but the replacement worker has not … that is the old container's final tick". The same-container re-verify deliberately does NOT require `started_at` to advance (a legal in-place re-verify stays legal — `started_at` is never an unconditional condition). Reasons distinguish: new heartbeat from replacement worker / from existing worker / old-worker-post-baseline-tick / stale / row missing.
- Real container identity (`deploy/lib.sh::worker_container_id`): "" when no RUNNING worker container exists, otherwise the FULL `docker inspect` ID — never a short ID, never an image-tag comparison (`TARGET_IMAGE != CURRENT_IMAGE` decides nothing about heartbeats). A Docker query failure is an error (exit 1), never a silent "". `install.sh`/`update.sh` capture `HEARTBEAT_BASELINE` + `PRE_WORKER_CONTAINER_ID` before `up -d --wait` and `POST_WORKER_CONTAINER_ID` after it, and pass all three to the verifier. `COMPOSE up` returning success without a determinable post-start worker identity is a verification FAILURE (exit 19) — never a default to "same container" (guard in lib.sh `verify_heartbeat` for install.sh, explicit guard before `wait_heartbeat` in update.sh, preserving the schema-aware 19 path).
- Legacy fallback kept semantically identical: `heartbeat_verify_accepts_container_ids()` probes the target image (module importable AND a 5-argv `verify` dies on a malformed baseline with a parse traceback exit 1; the pre-W05-AUDIT-2 module answers with its usage error exit 2, a module-less image fails the import — both route to the inline fallback, whose own logic never decides the verdict). The inline fallback implements the SAME rules verbatim: `pre_id != post_id` → `started_at` must advance past `baseline["started_at"]`, with the same reason taxonomy (pinned by unit tests on the fallback text and exercised for real in the smoke against a W05-AUDIT-era image).
- A09 evidence (new `backend/ops/report_job_evidence.py` + new read-only collector section "## scheduled weekly report jobs (A09 — report_jobs history, NOT weekly_reports)"): prints recent `report_jobs` rows `WHERE trigger='scheduled'` (`ORDER BY week_code DESC, created_at DESC`, recent 24) with week / trigger / status / attempts / expected / created / started / finished / on_schedule — the expected instant is the §4.1 Monday 00:10 Asia/Shanghai (period END + 10 min), everything shown in Asia/Shanghai wall clock, ±5 min loop tolerance annotated (60 s loop: no meaningless second-level requirement). Summary lines: `weeks with a succeeded scheduled job: …` and `two consecutive scheduled+succeeded weeks (A09): yes/no` (ISO-year wrap aware); manual jobs and error texts are never printed; strictly read-only. docs/ACCEPTANCE.md A09 rewritten: judge from `report_jobs` `created_at` history (append-only, survives regenerates); `weekly_reports` success row + DOCX remain corroboration only.
**Tests:** +11 unit (`test_ops_heartbeat_verify.py` 11→15: the race scenario pinned in both directions — old worker's post-baseline tick with replaced container FAILs, the replacement worker's own tick PASSes — plus generalized replaced-requires-`started_at`-advance, same-container PASS with unchanged `started_at`, fresh-install PASS under replacement, other-worker/missing row FAIL under either identity, container-identity decision matrix, CLI shapes for the 5-argv signature; `test_deploy_scripts.py` 9→11: `worker_container_id` = full inspect ID, PRE/POST captured around the (re)start in both scripts, three-argument verifier calls, missing post-start identity = hard failure in both install and update paths, capability probe + fallback invariants `pre_id != post_id` / `started <= baseline_started` / "replacement worker"; new `test_ops_report_job_evidence.py` 5: ±5 min tolerance edges, succeeded-week uniqueness/order, ISO-consecutive pairs incl. 53-week wrap, evidence-line field shape). +8 integration (`test_heartbeat_verify_persistence.py` 6→8: the exact race DATA sequence on real PostgreSQL — baseline T0/S0 → old worker's final tick T1>S0 at S0 → replaced container → FAIL → replacement worker's tick T2>T1/S1>S0 → PASS, plus the false-positive contrast that the same data PASSes when judged same-container, and a real-CLI test with the deploy scripts' exact argv; new `test_report_job_evidence.py` 6: manual regenerate overwrites `weekly_reports.generated_at` while the scheduled `report_jobs` row keeps its `created_at`/status and the collector output is unchanged with the manual job absent; manual-only week → no scheduled evidence; failed scheduled job listed but not counted; two consecutive scheduled+succeeded weeks identified; non-consecutive weeks not claimed; ISO wrap). Suite: **329 unit + 261 integration PASS**, ruff clean, mypy clean (136 files), `alembic upgrade head` idempotent at **0011 = head** (no new migration).  
**Acceptance (smoke evidence, staging roots on Debian 13 amd64, throwaway stack; images built from this tree, from 838b5b6, and a raising-0012 variant):** fresh install exit 0 — `heartbeat baseline: {}`, pre container `<none>`, post container = full 64-hex ID, PASS "first heartbeat" (mode A); same-image update exit 0 — PRE == POST (container unchanged), PASS "worker kept running" with `started_at` legitimately unmoved (mode B, real in-place re-verify); REAL image update `w05aud2-head` → `w05aud2-v1` exit 0 — container replaced, and since the target image predates the W05-AUDIT-2 signature the capability probe routed it to the inline fallback which produced the exact replacement-worker verdict (mode C via the fallback); rollback `--image w05aud2-head` exit 0 — replaced again, PASS via the tested module ("started_at advanced past …"). **Critical race, real containers:** baseline captured → old worker's genuine post-baseline tick (waited 40 s) → same-container verdict PASSed on it (demonstrating the false-positive enabler) → worker force-recreated into a SILENT container (`sleep 3600` override, new container ID) → verifier FAIL exit 1 with "old worker wrote after the baseline but the replacement worker has not … the old container's final tick" → real worker recreated, its own tick lands → verifier PASS exit 0 "by the replacement worker". **Other-worker:** with fresh rows for both 'worker' and 'other-worker' in the table, the verifier queried as 'deploy-other' FAILs "no heartbeat row for worker 'deploy-other'" — a foreign fresh row never stands in. Migration failure (`w05aud2-break`, raising 0012) → exit **14**, `.env` restored, container IDs unchanged, `/health` ok (old stack never restarted). Post-migration failure FOR REAL: heartbeat writes blocked by a trigger after a committed (idempotent) migration → same-image `update.sh` reached heartbeat verification and exited **19** printing the mandated schema-aware block with ZERO "rollback with:" lines; after removing the block, `update.sh` exit 0 "worker kept running". A09 evidence on the dev stack: collector section lists only `trigger=scheduled` rows (the week's manual jobs absent), off-schedule creation honestly `on_schedule=no`; a REAL manual regenerate of 2026-W35 (`request_regenerate` + `execute_report_job`, succeeded) moved `weekly_reports.generated_at` by ~2.4 h while the scheduled `report_jobs` row kept its original `created_at`/`succeeded` and the collector output was unchanged — manual regenerate does not destroy scheduled evidence; full collector run exit 0 with the new section.  
**Control state:** W05-T001..T004 remain REVIEW_PASSED (revalidated); W05-AUDIT = **REVIEW_FAILED — superseded by W05-AUDIT-2** (its fixes (2)/(3) stand; fix (1) resolved here); W05-T005 remains **IN_PROGRESS — REAL_ENVIRONMENT_EVIDENCE_PENDING** (A01–A16 all PENDING, no real-environment evidence fabricated); W01-T007 BLOCKED — FIELD_VALIDATION_PENDING; W03-T010 BLOCKED — REAL_WEEK_DATA_PENDING; W05-GATE **BLOCKED**; no formal two-week acceptance timing started; `main` NOT merged.  
**Checkpoint:** 78f4e3b2ef64daaab6ca39993aa28068294e499d (see EXECUTION_STATE.md Wave 5 checkpoint ledger).

## W05-PRE-ACCEPTANCE-HARDENING — pre-acceptance P2 cleanup
**Status:** REVIEW_PASSED（2026-09-06, branch work/wave-05）  
**Depends On:** W05-AUDIT-2  
**Blocks:** none (audit hardening only; no product capability added, NO new migration — alembic head stays 0011)  
**Scope:** exactly three P2 findings left before the formal two-week acceptance — no product features, no architecture changes, no acceptance timing started.  
**Implementation:**
- No-row baseline + existing-worker replacement (`backend/ops/heartbeat_verify.py`, `deploy/lib.sh`): W05-AUDIT-2 still treated a baseline of `{}` as "fresh install — the first fresh row is the evidence". But "no row yet" is also what the DB looks like when an EXISTING worker container simply has not ticked since it started: capture baseline `{}` → the OLD container writes its FIRST-ever tick after the capture → compose replaces it → the replacement worker stays silent → the old container's tick passed as "fresh install". The baseline now ALWAYS records `captured_at`; with no baseline row the verifier branches on the PRE identity: fresh install (no worker container before, `""` → new) still passes on the first fresh row; a REPLACED existing container (`pre != ""` and `pre != post`) requires `started_at` strictly AFTER `captured_at` (FAIL "old container wrote after the baseline but the replacement worker has not" otherwise); a LEGACY baseline without `captured_at` cannot attribute the row and FAILS closed with an actionable reason. A no-row baseline with the container UNCHANGED still passes on the first fresh row (the row can only come from the container that kept running). Baselines WITH a row keep the W05-AUDIT-2 rules unchanged.
- Baseline backward compatibility: `parse_baseline` accepts every historical format — `{}` and `{"last_heartbeat", "started_at"}` (pre-hardening) plus `{"captured_at"}` and the full current format — so a rollback target reading a pre-hardening baseline never crashes; a lone `started_at` (corrupt) is a ValueError, a lone `last_heartbeat` stays a KeyError. The lib.sh inline fallback BASELINE (serving pre-W05-AUDIT rollback targets) emits `captured_at` too, and the inline fallback VERIFIER implements the new no-row rules verbatim (fresh install / captured_at bound / legacy fail-closed / same-container) — kept in sync with the tested module.
- A09 on-time summary (`backend/ops/report_job_evidence.py`): `scheduled_succeeded_weeks()` counted ANY succeeded scheduled job, so two consecutive weeks created 00:20 (outside Monday 00:10 ±5 min) were summarized as the A09 shape, contradicting docs/ACCEPTANCE.md. Renamed to `scheduled_on_time_succeeded_weeks()`: a week counts only with `trigger=scheduled` (query-level) AND `status=succeeded` AND `is_on_schedule(item)`. Summary lines now `weeks with an on-time succeeded scheduled job: …` and `two consecutive on-time scheduled+succeeded weeks (A09): yes/no`; per-row evidence lines unchanged (still honest `on_schedule=no` for late rows); ISO-year wrap pairing unchanged; `weekly_reports`/DOCX remain corroboration only.
- update.sh schema-aware failure routing (`deploy/update.sh`): `POST_WORKER_CONTAINER_ID=$(worker_container_id)` failing after a committed migration used a bare `die 19`, losing the schema-aware remediation. Every failure past `alembic upgrade head` — heartbeat-baseline capture, PRE identity capture, restart (17), POST identity query, missing POST identity, web health (18), heartbeat (19) — now routes through `post_migration_failure`; the migration-failure path keeps `.env` restore + old stack untouched + `die 14`.
**Tests:** unit 329→340 (heartbeat verifier: no-row replacement rejects the old container's first tick / replacement worker PASS / no-row same-container PASS / legacy `{}` fail-closed with fresh-install + same-container compatibility / legacy + captured_at parsing matrix / corrupt partial baseline rejection; A09: two late → no, one on-time + one late → no, two on-time → yes, late success never shadows an on-time failed attempt, failed-even-on-time never counts, ISO wrap through the on-time summary; deploy scripts: no bare `die 1[789]` after the migration step, POST-ID query failure routed schema-aware, fallback no-row invariants `started <= captured_at` / `no captured_at` / fresh install / unchanged container, baseline fallback emits `captured_at`). Integration 261→266: the no-row race DATA sequence on real PostgreSQL (no-row baseline → old container's first tick started BEFORE the capture → replaced container → FAIL; legacy `{}` → fail-closed; replacement worker's own tick → PASS) and the real-CLI baseline/verify sequence with a stepped fixed clock (baseline emits only `captured_at`; old tick exit 1; replacement tick exit 0). Suite: **340 unit + 266 integration PASS**, ruff clean, mypy clean (136 files), `alembic upgrade head` idempotent at **0011 = head** (no new migration).  
**Acceptance (smoke evidence, staging roots, throwaway stack, images `w05hardening-head`/`-v2`/raising-`0012` built from this tree):** fresh install exit 0 — baseline `{"captured_at": …}` only, pre container `<none>`, PASS "first heartbeat … on a fresh install"; same-image update exit 0 — PRE == POST, PASS "worker kept running"; re-tagged-image update exit 0 — container replaced, PASS "by the replacement worker (started_at advanced past …)". **Critical no-row race, real containers:** heartbeat row deleted while the worker container kept running → no-row baseline captured → the OLD worker's genuine first post-baseline tick observed (last_heartbeat after `captured_at`, started_at before it) → worker force-recreated into a SILENT container (`sleep 3600` override, new ID) → verifier FAIL exit 1 "old container wrote after the baseline but the replacement worker has not … started_at <= captured_at"; legacy `{}` baseline on the same data FAIL closed exit 1; same-container contrast PASS exit 0; real worker restored → its own tick PASS exit 0 "started_at > captured_at". **A09 on the staging stack:** two late (00:20) scheduled successes → `none` / no; one on-time + one late → `2026-W36` / no; two consecutive on-time → yes (2026-W36 -> 2026-W37); two on-time MANUAL successes → no scheduled evidence at all; a manual regenerate moving `weekly_reports.generated_at` ~3 h20m left the scheduled `report_jobs` row and the collector output unchanged. **update.sh injections:** raising-0012 image → exit **14**, `.env` restored, container IDs unchanged; `docker inspect` query failure injected after a committed migration hit the POST identity capture → exit **19** with the full schema-aware block, ZERO "rollback with:" lines (same injection from the start hits the PRE capture → exit **19** schema-aware); heartbeat writes blocked by a trigger after a committed migration → restart + health OK then exit **19** schema-aware; trigger removed → normal update exit **0**, schema still 0011.  
**Control state:** W05-T001..T004 remain REVIEW_PASSED; W05-AUDIT = REVIEW_FAILED — superseded (history intact); W05-AUDIT-2 = REVIEW_PASSED; W05-T005 remains **IN_PROGRESS — REAL_ENVIRONMENT_EVIDENCE_PENDING** (A01–A16 all PENDING); W01-T007 BLOCKED — FIELD_VALIDATION_PENDING; W03-T010 BLOCKED — REAL_WEEK_DATA_PENDING; W05-GATE **BLOCKED**; formal two-week acceptance timing NOT started; `main` NOT merged.  
**Checkpoint:** 7c79d2c49951bb322456109491f4d25240a28880 (see EXECUTION_STATE.md Wave 5 checkpoint ledger).
...
**Engineering side ready (2026-09-06):** W05-T001–T004 REVIEW_PASSED on work/wave-05 (production layout, install.sh, update.sh, logging/retention/runbook); acceptance procedure and evidence tooling ready per docs/ACCEPTANCE.md (A01–A16); W05-AUDIT-2 REVIEW_PASSED (supersedes the W05-AUDIT heartbeat fix with the container-identity state machine; A09 evidence moved to report_jobs history; retention/rollback fixes of W05-AUDIT stand; alembic head 0011 unchanged).


## W05-LINUX-PORTABILITY — Linux deployment capabilities
